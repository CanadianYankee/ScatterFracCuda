...
extern cudaError_t cuda_test_generate(PVOID pArray, PVOID pMax, SIZE_2D size, size_t count);
...
HRESULT CGenerator::Initialize(ComPtr<ID3D11Device> pD3DDevice)
{
	...
	m_nAccumMargin = 0;
	size_t pitch;
	err = cudaMallocPitch(&m_pAccumArray, &pitch, m_sizeAccum.nWidth * sizeof(UINT), m_sizeAccum.nHeight);
	if (err != cudaSuccess) return E_FAIL;
	err = cudaMemset2D(m_pAccumArray, pitch, 0, m_sizeAccum.nWidth * sizeof(UINT), m_sizeAccum.nHeight);
	if (err != cudaSuccess) return E_FAIL;
	m_sizeAccum.nPitch = (UINT)pitch;

	err = cudaMallocManaged(&m_pAccumMax, sizeof(UINT));
	if (err != cudaSuccess) return E_FAIL;
	err = cudaMemset(m_pAccumMax, 0, sizeof(UINT));
	if (err != cudaSuccess) return E_FAIL;
...
HRESULT CGenerator::Iterate()
{
	...
	err = cuda_test_generate(m_pAccumArray, m_pAccumMax, m_sizeAccum, 0xffffff);
	if (err != cudaSuccess) return E_FAIL;

	((float*)m_pDrawScale)[0] = 1.0f / (float)((UINT*)m_pAccumMax)[0];
