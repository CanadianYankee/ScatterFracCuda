...
public:
	...
	__host__ __device__ inline UINT Length() { return m_nLength; }

	__host__ __device__ size_t ElementSize() { return sizeof(T); }

	__host__ __device__ inline bool ValidIndex(UINT idx) { return idx < m_nLength; }
	__host__ __device__ inline bool ValidIndex(int idx) { return idx >= 0 && idx < (int)m_nLength; }
...
public:
	...
	__host__ __device__ inline UINT Width() { return m_nWidth; }
	__host__ __device__ inline UINT Height() { return m_nHeight; }
	__host__ __device__ inline UINT Pitch() { return m_nPitch; }

	__host__ dim3 BlockDim(dim3 threadDim) {
	...
	__host__ __device__ inline bool ValidIndex(UINT idx, UINT idy) { return idx < m_nWidth && idy < m_nHeight; }
	__host__ __device__ inline bool ValidIndex(int idx, int idy) { return idx >= 0 && idy >= 0 && idx < (int)m_nWidth && idy < (int)m_nHeight; }
...
public:
	...
	__host__ __device__ inline UINT Width() { return CCudaArray2D<float>::Width() / 4; }
	__host__ __device__ inline UINT Height() { return CCudaArray2D<float>::Height(); }
	__host__ __device__ inline UINT Pitch() { return CCudaArray2D<float>::Pitch(); }

	__host__ __device__ inline bool ValidIndex(UINT idx, UINT idy) { return CCudaArray2D<float>::ValidIndex(idx * 4, idy); }
	__host__ __device__ inline bool ValidIndex(int idx, int idy) { return CCudaArray2D<float>::ValidIndex(idx * 4, idy); }
